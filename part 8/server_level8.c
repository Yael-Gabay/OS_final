...
void client_handler(int client_fd, struct shared_memory *shared_memory_ptr) {
    ...
    if (valread > 0) {
        ...
        if (shared_memory_ptr->request_counter % 100 == 0) {
            printf("Report: Process detected 100 numbers checked. The highest prime number so far is: %lld\n", shared_memory_ptr->highest_prime);
        }
